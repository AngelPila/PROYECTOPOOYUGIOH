...
def mostrar_tableros(tablero_jugador, tablero_bot):
    print(colored(f"\n{"-"*76}TABLERO{"-"*67}","red", attrs=["bold"]))
    print(f"\n{"-"*73}Tablero del Bot{"-"*62}\n")
    print(f"\n{'-'*150}\n")
    ...
    for carta in tablero_bot.esp_monster:
        ...
        if carta.area == CartaUbicacion.BDOWN:
            linea += f"|[{i}]. {carta.nombre} (boca abajo) | "
        else:
            if not isinstance(carta, Espacio):
                linea += f"|[{i}]. {carta.nombre}, ATK: {carta.ataque}, DEF: {carta.defensa} | "
                i += 1
            else:
                linea += f"| [{i}]. Vacio |\t \t \t"
                i += 1
...
def main(): 
    ...
    us=Deck([])
    bot_opa=Deck([])
    deck_usuario = us.CreacionDeck("lol")
    deck_bot = bot_opa.CreacionDeck("lol")
    ...
    while not jugador.jugadorDerrotado() and not bot.botDerrotado():
        ...
        if turno_jugador:
            ...
            while ataque == 0:
                ...
                if turno != 1 and len(jugador.cartas_mano) < 5:
                    jugador.TomarCarta()
                print("\nTurno del Jugador:")
                print("Tus cartas en mano:")
                for idx, carta in enumerate(jugador.cartas_mano):
                    if isinstance(carta,CartaMonstruo):
                        print(f"{idx + 1}. {carta.nombre} (MONSTRUO) ({carta.descripcion}) ATK:{carta.ataque} DEF:{carta.defensa}")
                    elif isinstance(carta,CartaMagica):
                        print(f"{idx + 1}. {carta.nombre} (MAGICA) ({carta.descripcion})")
                    elif isinstance(carta,CartaTrampa):
                        print(f"{idx + 1}. {carta.nombre} (TRAMPA) ({carta.descripcion})") 

                accion = input("Elige una acción: (1) Invocar carta, (2) Usar carta mágica, (3) Atacar, (4) Usar carta Trampa, (5) Pasar turno: ")
                ...
                if accion == "1":
                        ...
                        for m in tablero_jugador.esp_monster:
                            if not isinstance(m, Espacio) and carta.tipo_afectado == m.tipo_M:
                                ...
                                while posicion not in [1,2,3]:
                                    posicion = int(input("Elige posición (1-3): "))
                                carta.usoMag(m)
                                tablero_jugador.AgregarMag(carta, posicion)
                                jugador.cartas_mano.remove(carta)
                    ...
                    indice_idx = str(input("Elige el índice de la carta Trampa a usar: "))
                    while indice_idx not in["1","2","3","4","5"]:
                        indice_idx = input("Elige un indice valido: ")
                    idx = int(indice_idx) - 1
                    carta = jugador.cartas_mano[idx]
                    if isinstance(carta,CartaTrampa):
                        ...
                        while posicion not in [1,2,3]:
                            posicion = int(input("Elige posición (1-3): "))
                        tablero_jugador.AgregarTrap(carta,posicion)
                        jugador.cartas_mano.remove(carta)
                    else:
                        print("No es una carta trampa")
