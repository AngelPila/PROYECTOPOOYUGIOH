...
class Jugador:
    ...
    def Atacar(self, tablero_oponente, tablero, carta_atacante):
        ...
        if objetivos and isinstance(carta_atacante, CartaMonstruo):
            ...
            if carta_atacante.ataque > objetivo.defensa:
                ...
                daño_real  = carta_atacante.ataque - objetivo.defensa
                return daño_real
                ...
                daño_real  = objetivo.defensa -carta_atacante.ataque 
                self.hp -= daño_real
...
class CartaTrampa(Carta):
    ...
    def usoTram(self, carta):
        if self.atributo_afectado == carta.tipo_A:
            carta.ataque=0
...
class Tablero:
    ...
    def AgregarAtk(self, carta, posicion):
        if isinstance(self.esp_monster[posicion - 1], Espacio):
            carta1=self.clonar_carta(carta)
            carta1.area = CartaUbicacion.BUP
            self.esp_monster[posicion - 1] = carta1
            

    def AgregarDef(self, carta, posicion):
        if isinstance(self.esp_monster[posicion - 1], Espacio):
            carta1=self.clonar_carta(carta)
            carta1.area = CartaUbicacion.BDOWN
            self.esp_monster[posicion - 1] = carta1
    ...
    def AgregarTrap(self, carta, posicion):
        if isinstance(self.esp_magtrp[posicion - 1], Espacio):
            carta1=self.clonar_carta(carta)
            carta1.area = CartaUbicacion.BDOWN
            self.esp_magtrp[posicion - 1] = carta1
...
class Bot(Jugador):
    ...
    def atacarConMonstruos(self, tablero_bot, oponente, ataque):
        ...
        while i==0:
            ...
            if isinstance(carta_atacante, CartaTrampa):
                if carta_atacante.atributo_afectado == CartaTrampa.atributo_afectado:
                    carta_atacante.ataque=0
                    print(f"El bot se ha encontrado con una trampa su ataque de {carta_atacante.ataque} ha sido inhabilitado")

            elif not isinstance(carta_atacante, Espacio):
